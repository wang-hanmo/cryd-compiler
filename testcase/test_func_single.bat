@echo off
Set filename=%1
echo %filename%>> func_test_result.txt

:: 编译ir2c
cp .\functional\%filename%.sy %filename%.c
...
if exist ir2c_%filename%.exe (
     if exist .\functional\%filename%.in (
        cat .\functional\%filename%.in | .\ir2c_%filename%.exe >> ir2c_%filename%.out
    ) else (
...
cp .\functional\%filename%.out .\%filename%.out
